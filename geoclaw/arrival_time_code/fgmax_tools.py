...
class FGmaxGrid(object):
    ...
    def __init__(self):
        ...
        self.tfirstPOS = None
        self.tfirstNEG = None
        self.tfirstDRAW = None
        self.tfirstADVIS = None
        self.tfirstWARN = None
    ...
    def read_fgmax_grids_data(self, fgno, data_file='fgmax_grids.data'):
        ...
        self.mhta = int(fgmax_input[5].split()[0])
        self.arrival_tols_h = [float(tol) for tol in \
                               fgmax_input[6].split()[:self.mhta]]
        self.interp_method = int(fgmax_input[5+2].split()[0])
        self.point_style = point_style = int(fgmax_input[6+2].split()[0])
        print('Reading input for fgno=%i, point_style = %i ' \
                % (self.fgno, self.point_style))
        if point_style == 0:
            self.npts = npts = int(fgmax_input[7+2].split()[0])
            if npts == 0:
                self.xy_fname = fgmax_input[8+2][1:-2]  # strip quotes
            ...
            self.npts = npts = int(fgmax_input[7+2].split()[0])
            self.x1 = float(fgmax_input[8+2].split()[0])
            self.y1 = float(fgmax_input[8+2].split()[1])
            self.x2 = float(fgmax_input[9+2].split()[0])
            self.y2 = float(fgmax_input[9+2].split()[1])
        elif point_style == 2:
            self.nx = nx = int(fgmax_input[7+2].split()[0])
            self.ny = ny = int(fgmax_input[7+2].split()[1])
            self.x1 = float(fgmax_input[8+2].split()[0])
            self.y1 = float(fgmax_input[8+2].split()[1])
            self.x2 = float(fgmax_input[9+2].split()[0])
            self.y2 = float(fgmax_input[9+2].split()[1])
        elif point_style == 3:
            self.n12 = n12 = int(fgmax_input[7+2].split()[0])
            self.n23 = n23 = int(fgmax_input[7+2].split()[1])
            self.x1 = float(fgmax_input[8+2].split()[0])
            self.y1 = float(fgmax_input[8+2].split()[1])
            self.x2 = float(fgmax_input[9+2].split()[0])
            self.y2 = float(fgmax_input[9+2].split()[1])
            self.x3 = float(fgmax_input[10+2].split()[0])
            self.y3 = float(fgmax_input[10+2].split()[1])
            self.x4 = float(fgmax_input[11+2].split()[0])
            self.y4 = float(fgmax_input[11+2].split()[1])
        elif point_style == 4:
            self.xy_fname = fgmax_input[7+2][1:-2]  # strip quotes
    ...
    def write_to_fgmax_data(self, fid):
        ...
        if self.arrival_tol is None:
            self.arrival_tol = 9999999.
        fid.write("%12.3f                # arrival_tol\n" % self.arrival_tol)
        if self.arrival_tols_h is None:
            self.arrival_tols_h = 9999999.
        if type(self.arrival_tols_h) in [float,int]:
            self.arrival_tols_h = tuple([self.arrival_tols_h])
        else:
            self.arrival_tols_h = tuple(self.arrival_tols_h)
        mhta = len(self.arrival_tols_h)
        fid.write("%3i %s              # mhta\n"  % (mhta, 10*" "))
        format = "%12.3f" + (mhta-1)*"  %8.3f" + "    # arrival_tols_h\n"
        fid.write(format % self.arrival_tols_h)
    ...
    def read_output(self, fgno=None, outdir=None, verbose=True, 
        ...
        # with 5 arrival times and h0:
        cols_expected = [12,14,20]
        # with 6 arrival times and h0:
        cols_expected = [13,15,21]
        ...
        # last columns is arrival times:
        mhta = len(self.arrival_tols_h)
        self.arrival_time = numpy.ma.empty([1+mhta]+list(fg_shape))
        for mt in range(mhta+1):
            arrival_time = numpy.reshape(d[:,ind_arrival_time+mt],
            ...
            self.arrival_time[mt,...] = arrival_time
        ...
        self.B = B
        self.h = h
        self.h0 = numpy.reshape(d[:,ind_arrival_time+mhta+1],
                                fg_shape,order=reshape_order)
    ...
    def ps4_to_arrays(self, verbose=True):
        ...
        zarrays = ['level','B','h','h_time','s','s_time','hs','hs_time',\
                   'hss','hss_time','hmin','hmin_time','arrival_time','h0']
        ...
        for attr in zarrays:
            ...
            if z_1d is None:
                ...
                if z_1d.ndim == 1:
                    ...
                    # arrival_time is now an array for different thresholds
                    mhta = len(self.arrival_tols_h)
                    assert z_1d.shape[0] == mhta+1, \
                        '*** unexpected z_1d shape: %s' % str(z_1d.shape)
                    Zshape = [z_1d.shape[0]]+list(X.shape)
                    Z = ma.masked_array(data=numpy.empty(Zshape), mask=True)
                    for mt in range(z_1d.shape[0]):
                        for k in range(len(x_1d)):
                            i = int(round((x_1d[k]-x1)/dx))
                            j = int(round((y_1d[k]-y1)/dy))
                            Z[mt,j,i] = z_1d[mt,k]                        
